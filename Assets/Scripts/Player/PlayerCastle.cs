...
public class PlayerCastle : MonoBehaviour, IDamageable
{
    ...
    {
        Debug.Log($"[PlayerCastle] HP depleted! CurrentHealth: {CurrentHealth}, IsDead: {IsDead}");
        ...
        // ゲームオーバーイベントを発火（GameManagerが実装されたら使用）
        // GameManager.OnPlayerCastleDestroyed?.Invoke();
    }
    ...
    private void PlayDestroyedSound()
    {
        if (audioSource != null && destroyedSound != null)
        {
            ...
            Debug.Log("[PlayerCastle] Destroyed sound played.");
        }
        else
        {
            if (audioSource == null)
            {
                Debug.LogWarning("[PlayerCastle] AudioSource is not set. Destroyed sound will not play.");
            }
            if (destroyedSound == null)
            {
                Debug.LogWarning("[PlayerCastle] Destroyed sound AudioClip is not set. Please assign Destroyed Sound clip in Inspector.");
            }
        }
    }
