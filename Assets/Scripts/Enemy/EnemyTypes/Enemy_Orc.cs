...
public class Enemy_Orc : MonoBehaviour
{
    ...
    private void CheckForTargetsAndAttack()
    {
        ...
        if (target != null)
        {
            ...
            if (enemyController != null)
            {
                bool wasMoving = enemyController.IsMoving;
    ...
    private IDamageable FindNearestTarget()
    {
        ...
        if (attackRange <= 0f)
        {
            Debug.LogWarning($"[Enemy_Orc] AttackRange is {attackRange}. Please set a valid attack range in EnemyData asset.");
            return null;
        }
        ...
        foreach (Collider2D col in colliders)
        {
            ...
            else if (col.CompareTag("PlayerCastle"))
            {
                ...
                {
                    Debug.LogWarning($"[Enemy_Orc] IDamageable component not found on {col.gameObject.name} with PlayerCastle tag!");
                }
            }
        }
