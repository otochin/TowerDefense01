...
/// <summary>
/// ファイヤーボールエフェクト
/// Mageから敵に向かって飛ぶファイヤーボールのビジュアルエフェクト
/// </summary>
[RequireComponent(typeof(SpriteRenderer))]
public class FireballEffect : MonoBehaviour
{
    [Header("エフェクト設定")]
    [SerializeField] private float moveSpeed = 15f; // 移動速度
    ...
    /// エフェクトを初期化
    /// </summary>
    /// <param name="from">開始位置（Mageの位置）</param>
    /// <param name="to">目標位置（敵の位置）</param>
    /// <param name="fireballSprite">ファイヤーボールのスプライト</param>
    /// <param name="effectScale">エフェクトのサイズ（オプション）</param>
    public void Initialize(Vector3 from, Vector3 to, Sprite fireballSprite = null, Vector3? effectScale = null)
    {
        ...
        // スプライトが指定されている場合は設定
        if (fireballSprite != null && spriteRenderer != null)
        {
