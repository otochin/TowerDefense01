...
14. ゲーム終了後の処理：
    - **勝利時**：キャラクター強化UIが表示される（6つのボタンから1つだけ選択可能：Warrior、Archer、MageのHP強化3つ + Attack Power強化3つ）。ゲームモード選択パネルは非表示。強化ボタンをクリックすると、強化が適用され、現在のゲームモードのまま次のステージへ進む（ゲームモード選択パネルは表示しない）
    - **敗北時**：ゲームモード選択パネルが表示される。キャラクター強化UIは非表示。ゲームモードを選択すると、ステージ1からゲームが再開される（Power・城のHPがリセット、フィールド上のキャラクター・エネミーが削除される）
...
- **プレイヤー城**: 左側に配置。敵が到達するとダメージを受ける。攻撃を受けた時に一瞬赤くフラッシュする視覚的フィードバック。ダメージを受けた時にダメージ数値が表示される（キャラクターと同じDamagePrefabを使用）。HPが0になると「Lost!」が赤で表示され、ゲーム終了
- **敵城**: 右側に配置。プレイヤーキャラクターが攻撃対象。攻撃を受けた時に一瞬赤くフラッシュする視覚的フィードバック。ダメージを受けた時にダメージ数値が表示される（キャラクターと同じDamagePrefabを使用）。HPが0になると「Win!」が緑で表示され、ゲーム終了
- **城のHPバー**: 背景バーとFillバーで構成。FillバーはHPに応じて減少する
- **道**: 両陣営の間にある進軍ルート
- **キャラクター**: 様々な種類のユニットを召喚可能。敵キャラクターだけでなく敵の城にも攻撃可能。攻撃時に突進アニメーションが発生。ダメージを受けた時にダメージ数値が表示される（DamagePrefabを使用、上下・左右のオフセットをInspectorで調整可能）
- **キャラクター選択UI**: 画面下部にキャラクター選択パネルを表示。3つのキャラクター（Warrior、Archer、Mage）から選択可能。各ボタンにはアイコン、コスト表示、アニメーション付き。Powerが不足している場合はボタンが無効になり、アニメーションが停止する視覚的フィードバックを提供
- **敵キャラクター**: 自動的に左方向へ進軍し、プレイヤーキャラクターと戦闘。プレイヤーの城にも攻撃可能。攻撃時に突進アニメーションが発生。ダメージを受けた時にダメージ数値が表示される（DamagePrefabを使用、上下・左右のオフセットをInspectorで調整可能）
- **ゲーム終了処理**: 城のHPが0になると、タイマー・スポーン・キャラクター・エネミーの動きが停止し、「Win!」または「Lost!」が表示される。**勝利時**：キャラクター強化UIが表示され、ゲームモード選択パネルは非表示。**敗北時**：ゲームモード選択パネルが表示され、キャラクター強化UIは非表示
- **キャラクター強化システム**: ゲーム終了時にキャラクター強化UIが表示される。6つのボタン（Warrior、Archer、MageのHP強化3つ + Attack Power強化3つ）から1つだけ選択して強化できる。強化タイプはHP、攻撃力、攻撃速度、移動速度。各キャラクターごとに強化値をInspectorで設定可能（Warrior、Archer、Mageそれぞれで異なる強化量を設定できる）。強化ボタンのラベルには実際の設定値が表示される。**勝利時**：キャラクター強化UIが表示され、ゲームモード選択パネルは非表示。強化ボタンをクリックすると、強化が適用され、現在のゲームモードのまま次のステージへ進む（ゲームモード選択パネルは表示しない）。**敗北時**：ゲームモード選択パネルが表示され、キャラクター強化UIは非表示。ゲームモードを選択すると、ステージ1からゲームが再開される。CharacterUpgradePanel表示中はCharacterSelectPanelを自動的に非表示にする
...
| 2026-01-24 | キャラクター選択UI仕様追加：キャラクター選択ボタンのアニメーション制御機能を仕様書に追加（お金不足時はアニメーション停止、十分時はアニメーション実行）。ゲーム要素セクションにキャラクター選択UIの詳細な説明を追加 | - |
