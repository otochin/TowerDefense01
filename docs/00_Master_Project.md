...
17. ステージシステム：画面右上に現在のステージ数を表示（「Stage: 1」など）。最初は「Stage: 1」。勝利した後、ゲームモードを選ぶとStageが１つ進む。Stageが１つ進むごとに、モンスターが表示される秒数が0.9倍になる（徐々に短くなって難易度が上がっていく）。負けた後、ゲームモードを選ぶとStage1からやり直しになる
...
2026年1月20日（背景システム実装完了：ステージごとの背景切り替え機能、Background GameObjectのSpriteRendererを使用した背景表示機能を実装）
